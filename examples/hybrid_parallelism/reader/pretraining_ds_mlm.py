...
def make_pretrain_dataset(name, gz_files, is_train, vocab, batch_size, vocab_size, max_seqlen, global_rank, world_size):
    ...
    def sample_negative(dataset):
        def gen():
            ...
            while True:
                ...
                #log.debug(a)
                #log.debug(b)
                sample, seg_info, token_type = build_pair(a, b, max_seqlen, vocab) #negative sample might exceed max seqlen
    ...
    log.info('Apply sharding in distribution env %d/%d' % (global_rank, world_size))
